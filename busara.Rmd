...
d.fls <- batch_pubmed_download(
  pubmed_query_string = paste(
    ...
    "AND ('2008/01/01'[Date - Publication] : '3000/12/31'[Date - Publication])" 
  ),
...
Possible future steps include: (a) obtaining a better, more current university name database (that includes country of university), (b) making manual corrections for other research institutes not included in the university database, (c) finding a way around the API limit to get data for more years, (d) host DT tables on a server to speed up the website and allow the inclusion of a DT table for exploring the raw data, and (e) find a way to use country flags for the countries-by-journal figure.

(PubMed API Key character length = `r nchar(API_TOKEN_PUBMED)`)
