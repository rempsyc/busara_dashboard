...
```{r data_download, message=FALSE, warning=FALSE, results='hide'}
...
d.fls <- batch_pubmed_download(
  pubmed_query_string = paste(
    ...
    "AND ('2010/01/01'[Date - Publication] : '3000/12/31'[Date - Publication])" 
  ),
  dest_file_prefix = "data/easyPubMed_data_",
  batch_size = 5000,
  api_key = secrets.API_TOKEN_PUBMED)

```

```{r all_articles_to_df, message=FALSE, warning=FALSE}
# Convert XLM data to a data frame of first authors
articles.df <- table_articles_byAuth(d.fls, included_authors = "first")

articles.df_original <- articles.df

```

```{r address_split, message=FALSE, warning=FALSE}
