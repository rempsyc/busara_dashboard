...
save_process_pubmed_batch(
  journal = head(journal_field$journal_short, -1),
...
The full current list of journals queried can be obtained through the following R code `head(pubmedDashboard::journal_field$journal_short[-23], -1)` (note that PLOS One [the last observation] was excluded because its number of papers is too large to handle for this dashboard):

```{r, eval=F}
head(journal_field$journal_short, -1)
```
...
pubmedDashboard::journal_field$journal_short[
  !pubmedDashboard::journal_field$journal_short %in% 
    unique(articles.df4$journal)]
...
articles.df4 <- articles.df4 %>% 
  group_by(journal) %>% 
  mutate(first_Year = first(year), last_year = last(year),
         year_range = paste0(first_Year, "-", last_year)) %>% 
  ungroup()

count(articles.df4, journal, year_range, sort = TRUE) %>% 
  as.data.frame() %>%
